// ============================================================
// main.js — i18n (ES default / EN) + progressive contact form.
// ES lives in the HTML (works without JS, good for SEO); only the
// EN strings live here. ponytail: no framework, no router, plain DOM.
// ============================================================
(function () {
  'use strict';

  // --- EN dictionary (ES counterpart is the HTML's own text) ---------------
  var EN = {
    skip: 'Skip to content',
    lang_group: 'Language selector',
    nav_career: 'Career', nav_projects: 'Projects', nav_about: 'About', nav_skills: 'Experience',
    nav_contact: 'Contact', nav_cta: 'Get in touch', nav_toggle: 'Menu',
    theme_toggle: 'Toggle light/dark theme',
    hero_role: 'Full-stack web developer & lab technician',
    hero_lead: 'A scientific mindset applied to code. Specialised in Angular, Flask and Spring Boot, with a focus on precision, traceability and serious testing. Junior with projects deployed to real production.',
    hero_signature: 'A technical builder who backs up claims with public, deployed projects.',
    hero_cta_career: 'View career',
    sec_career: 'Career', sec_projects: 'Projects', sec_about: 'About', sec_skills: 'Experience', sec_contact: 'Contact',
    proj_ss_desc: 'Multi-tenant full-stack PropTech platform in real production. A satellite layer over vacation-rental PMS that automates operational tasks: apartments master data, operational heatmap, late check-in alerts, contact sync and an AI-assisted communications vault.',
    proj_dr_desc: 'Full-stack app to manage music collections, rate albums and write reviews, integrated with the Deezer API. JWT auth, role system and Swagger/OpenAPI docs.',
    proj_tk_desc: 'Interactive map built as a team during the DAW course. Docker containerisation, Vite-optimised build and a professional workflow with issues, milestones and pull requests.',
    metric_backend: '92 backend tests', metric_frontend: '297 frontend tests',
    metric_coverage: '90% coverage', status_team: 'Team', tag_teamwork: 'Teamwork',
    about_p1: 'I’m a full-stack developer, trained through the Web Application Development (DAW) programme. Right now I’m developing projects, and Stay Sidekick is already in production.',
    about_p2: 'Before that I spent two years and five months as a receptionist for a vacation-rental management company. That’s where the idea for Stay Sidekick was born, though the project would come later.',
    about_p3: 'And before the receptionist job, I was a higher technician in clinical and biomedical laboratory, with an internship in the blood bank at Hospital Puerta del Mar: sample analysis, strict protocols and traceability, like any lab technician in healthcare.',
    about_softskills_title: 'How I work',
    soft_org: 'Organisation', soft_resp: 'Responsibility', soft_creativity: 'Creativity',
    soft_adapt: 'Adaptability', soft_teamwork: 'Teamwork', soft_projmgmt: 'Project management',
    soft_frustration: 'Tolerance for frustration', soft_customer: 'Customer service', soft_flex: 'Flexibility',
    soft_pressure: 'Problem-solving under pressure',
    skills_languages: 'Languages', skills_frameworks: 'Frameworks & libraries',
    skills_databases: 'Databases', skills_devops: 'DevOps & infrastructure',
    skills_lab: 'Clinical lab', skills_spoken_languages: 'Languages spoken',
    lab_hematology: 'Haematology', lab_biochem: 'Clinical biochemistry',
    lab_micro: 'Microbiology', lab_immuno: 'Immunology',
    lab_bloodbank: 'Blood bank', lab_biosafety: 'Biosafety',
    lang_es: 'Spanish (native)', lang_en: 'English (B2)',
    contact_intro: 'A job offer, a collaboration or a technical question? Drop me a line and I’ll reply as soon as possible.',
    form_name: 'Name', form_email: 'Email', form_type: 'Inquiry type',
    form_type_job: 'Job offer', form_type_collab: 'Collaboration', form_type_other: 'Other',
    form_message: 'Message', form_submit: 'Send message',
    form_name_ph: 'Your name', form_email_ph: 'you@email.com', form_message_ph: 'Tell me how I can help…',
    channel_email: 'Direct email', channel_location: 'Location', channel_location_value: 'Cádiz, Spain',
    footer_copy: '© 2026 Sergio Durán Utrera · Cádiz, Spain',
    // CV timeline
    personal_tagline: 'Clinical lab technician turned full-stack web developer.',
    meta_location: 'Location', meta_location_v: 'Cádiz, Andalusia, Spain',
    meta_languages: 'Languages', meta_languages_v: 'Spanish native · English B2',
    meta_profile: 'Profile',
    card_studies: 'Education',
    edu1_role: 'Higher Technician in Web Application Development (DAW)',
    edu1_desc: 'Front/back development, REST APIs, relational databases, cloud deployment, UI/UX, accessibility, Git and agile methodologies. Programme completed.',
    edu2_role: 'Higher Technician in Clinical & Biomedical Laboratory',
    edu2_desc: 'Clinical biochemistry, microbiology, haematology, immunology, quality control, biosafety and LIS systems.',
    edu3_role: 'Baccalaureate — Science track',
    card_experience: 'Professional experience',
    exp1_role: 'Receptionist / Customer Service',
    exp1_org: 'Vacation Rental · Tourism & Social Innovation, Cádiz',
    exp1_desc: 'Booking management in PMS, multichannel customer service, payments and tourism regulations. Origin of Stay Sidekick.',
    exp2_role: 'Lottery Promoter', exp2_org: 'Red Cross, Cádiz',
    exp2_desc: 'In-person sales, payment handling and daily cash reconciliation.',
    exp3_role: 'Laboratory Technician (internship)',
    exp3_desc: 'Blood bank: reception, identification, centrifugation, separation and labelling of components under strict biosafety.',
    card_volunteer: 'Volunteering & international',
    vol1_role: 'Video game writer', vol1_org: 'Evelon Games (remote)',
    vol1_desc: 'Writing news, reviews and guides; remote coordination under deadlines.',
    vol2_role: 'Sociocultural facilitator', vol2_org: 'European Solidarity Corps · Ostrava (Czechia)',
    vol2_desc: 'Running youth activities in an international team. Youthpass certificate.',
    card_certs: 'Certifications & courses',
    cert1_desc: 'Verifiable credential on Credly.',
    cert2_role: 'Turiscool courses (HORECA)',
    cert2_desc: 'AI in business, first aid, fire prevention, equality and circular economy in tourism.',
    cert3_role: 'Workplace risk prevention (office)',
    cert4_role: 'Laboratory Assistant', cert4_org: 'Cádiz Chamber of Commerce',
    cert5_org: 'European Commission',
    cert_badge: 'Verify badge ↗'
  };

  // status messages need both locales (not present in the DOM)
  var MSG = {
    es: { sending: 'Enviando…', ok: '¡Mensaje enviado! Gracias.',
          error: 'No se pudo enviar. Escríbeme a duran.utrerasergio2612@gmail.com' },
    en: { sending: 'Sending…', ok: 'Message sent! Thanks.',
          error: 'Couldn’t send. Email me at duran.utrerasergio2612@gmail.com' }
  };

  // attr dataset-key -> DOM property/attribute
  var ATTRS = [
    ['i18n', 'textContent'],
    ['i18nPlaceholder', 'placeholder'],
    ['i18nAriaLabel', 'aria-label']
  ];

  var lang = localStorage.getItem('lang') === 'en' ? 'en' : 'es';

  function applyEl(el) {
    for (var i = 0; i < ATTRS.length; i++) {
      var dsKey = ATTRS[i][0], prop = ATTRS[i][1], key = el.dataset[dsKey];
      if (!key) continue;
      var cache = '_es_' + dsKey;
      if (el[cache] === undefined) {
        el[cache] = prop === 'textContent' ? el.textContent : (el.getAttribute(prop) || '');
      }
      var val = lang === 'es' ? el[cache] : (EN[key] != null ? EN[key] : el[cache]);
      if (prop === 'textContent') el.textContent = val;
      else el.setAttribute(prop, val);
    }
  }

  function setLang(next) {
    lang = next;
    localStorage.setItem('lang', lang);
    document.documentElement.lang = lang;
    document.querySelectorAll('[data-i18n], [data-i18n-placeholder], [data-i18n-aria-label]').forEach(applyEl);
    document.querySelectorAll('.c-lang__btn').forEach(function (b) {
      b.setAttribute('aria-pressed', String(b.dataset.lang === lang));
    });
  }

  document.querySelectorAll('.c-lang__btn').forEach(function (b) {
    b.addEventListener('click', function () { setLang(b.dataset.lang); });
  });

  setLang(lang); // caches ES originals + applies stored preference

  // --- Dark / light theme toggle (initial theme set by inline <head> script) -
  var themeBtn = document.querySelector('.c-theme');
  if (themeBtn) {
    var root = document.documentElement;
    var syncTheme = function () {
      themeBtn.setAttribute('aria-pressed', String(root.dataset.theme === 'dark'));
    };
    syncTheme();
    themeBtn.addEventListener('click', function () {
      var next = root.dataset.theme === 'dark' ? 'light' : 'dark';
      root.dataset.theme = next;
      localStorage.setItem('theme', next);
      syncTheme();
    });
  }

  // --- Mobile hamburger menu ------------------------------------------------
  var nav = document.querySelector('.c-nav');
  var navToggle = nav && nav.querySelector('.c-nav__toggle');
  if (navToggle) {
    var setMenu = function (open) {
      nav.classList.toggle('is-open', open);
      navToggle.setAttribute('aria-expanded', String(open));
    };
    navToggle.addEventListener('click', function () {
      setMenu(!nav.classList.contains('is-open'));
    });
    nav.querySelectorAll('.c-nav__link').forEach(function (l) {
      l.addEventListener('click', function () { setMenu(false); });
    });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape') setMenu(false);
    });
  }

  // --- Contact form: progressive enhancement --------------------------------
  var form = document.querySelector('.c-form');
  if (form) {
    var status = form.querySelector('.c-form__status');
    var setStatus = function (text, state) {
      status.textContent = text;
      status.setAttribute('data-state', state || '');
    };
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      if (!form.checkValidity()) { form.reportValidity(); return; }
      var endpoint = form.dataset.endpoint;
      if (!endpoint) { setStatus(MSG[lang].error, 'error'); return; }
      setStatus(MSG[lang].sending, '');
      var data = Object.fromEntries(new FormData(form).entries());
      fetch(endpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(function (r) {
        if (!r.ok) throw new Error();
        form.reset();
        setStatus(MSG[lang].ok, 'ok');
      }).catch(function () { setStatus(MSG[lang].error, 'error'); });
    });
  }
})();
